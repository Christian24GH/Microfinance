...
include __DIR__.'/config/bm_budadj_conn.php';
include __DIR__.'/config/bm_budadj_btnfunc.php';
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header bg-primary text-white">
                            <h1 class="modal-title fs-5" id="editdataLabel">Edit Budget Adjustment</h1>
                        </div>
                        <form action="../modal_function/bm_budadjfunc.php" method="POST">
                            <div class="modal-body">
                                <div class="mb-3">
                                    <input type="hidden" name="adjustment_id" id="adjustment_id" class="form-control" placeholder="Adjustment ID">
                                </div>

                                <div class="mb-3">
                                <label for="adjustmentAmount" class="form-label">Adjustment Amount</label>
                                    <input type="text" name="adjustmentAmount" id="adjustmentAmount" class="form-control" placeholder="Adjustment Amount">
                                </div>

                                <div class="mb-3">
                                <label for="adjustmentDate" class="form-label">Date</label>
                                    <input type="text" name="adjustmentDate" id="adjustmentDate" class="form-control" placeholder="Date">
                                </div>

                                <div class="mb-3">
                                <label for="reasonCode" class="form-label">Reason Code</label>
                                    <input type="text" name="reasonCode" id="reasonCode" class="form-control" placeholder="Reason Code">
                                </div>
                            </div>
                ...
                <div class="col">
                    <div class="card mt-2">
                        <div class="card-header">
                            <h2 class="text-center display-7">Budget Adjustment</h2>
                                ...
                                <tbody>
                                    <?php if (isset($result) && mysqli_num_rows($result) > 0): ?>
                                        <?php while ($row = mysqli_fetch_assoc($result)): ?>
                                            <tr>
                                                <td class="adjustment_id"><?php echo $row["adjustment_id"]; ?> </td>
                                                <td><?php echo $row["adjustmentAmount"]; ?> </td>
                                                <td><?php echo $row["adjustmentDate"]; ?> </td>
                                                <td><?php echo $row["reasonCode"]; ?> </td>
                                                <td>
...
<script>
    ...
    $(document).ready(function() {
        $('.delete_btn').click(function(e) {
            e.preventDefault();
            var adjustment_id = $(this).closest('tr').find('.adjustment_id').text();
            $('#delete_id').val(adjustment_id);
            $('#deleteModal').modal('show');
        });

        $('#deleteForm').submit(function(e) {
            e.preventDefault();
            var adjustment_id = $('#delete_id').val();
            $.ajax({
                method: "POST",
                url: "../modal_function/bm_budadjfunc.php",
                data: {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $.each(response, function(key, value) {
                        ...
                        $('#adjustment_id').val(value['adjustment_id']);
                        $('#adjustmentAmount').val(value['adjustmentAmount']);
                        $('#adjustmentDate').val(value['adjustmentDate']);
                        $('#reasonCode').val(value['reasonCode']);
                    });
                    $('#editdata').modal('show');
                }
            });
