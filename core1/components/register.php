...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql_accounts = "INSERT INTO accounts (email, password, role_id, client_id) VALUES (?, ?, ?, ?)";
    $stmt_accounts = $conn->prepare($sql_accounts);
    $stmt_accounts->bind_param("ssis", $email, $password, $role_id, $client_id);
    ...
    $sql_employment = "INSERT INTO client_employment (client_emp_id, client_id, employer_name, address, position) VALUES (?, ?, ?, ?, ?)";
    $stmt_employment = $conn->prepare($sql_employment);
    $stmt_employment->bind_param("issss", $client_emp_id, $client_id, $employer_name, $employment_address, $position);
    ...
    $sql_references = "INSERT INTO client_references (client_ref_id, client_id, fr_first_name, fr_last_name, fr_relationship, fr_contact_number, sr_first_name, sr_last_name, sr_relationship, sr_contact_number) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt_references = $conn->prepare($sql_references);
    $stmt_references->bind_param("isssssssss", $client_ref_id, $client_id, $fr_first_name, $fr_last_name, $fr_relationship, $fr_contact_number, $sr_first_name, $sr_last_name, $sr_relationship, $sr_contact_number);
    ...
    $sql_financial = "INSERT INTO client_financial_info (client_fin_id, client_id, source_of_funds, monthly_income) VALUES (?, ?, ?, ?)";
    $stmt_financial = $conn->prepare($sql_financial);
    $stmt_financial->bind_param("isss", $client_fin_id, $client_id, $source_of_funds, $monthly_income);
    $stmt_financial->execute();

    // Client Document Information
    $docu_type = $_POST["docu_type"];
    $document_one = $_FILES['document_one']['name'];
    $document_two = $_FILES['document_two']['name'];

    // Define upload directory
    $upload_dir = "../components/files/";

    // Move uploaded files to the upload directory
    if (
        move_uploaded_file($_FILES['document_one']['tmp_name'], $upload_dir . $document_one) &&
        move_uploaded_file($_FILES['document_two']['tmp_name'], $upload_dir . $document_two)
    ) {
        // Insert data into client_documents table
        $stmt = $conn->prepare("INSERT INTO client_documents (client_id, docu_type, document_one, document_two) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("ssss", $client_id, $docu_type, $document_one, $document_two);
    ...
    // Insert into client_status table
    $sql_status = "INSERT INTO client_status (client_id, c_status, l_status, r_status) VALUES (?, ?, ?, ?)";
    $stmt_status = $conn->prepare($sql_status);
    $stmt_status->bind_param("ssss", $client_id, $c_status, $l_status, $r_status);
    $stmt_status->execute();
    $stmt_status->close();
